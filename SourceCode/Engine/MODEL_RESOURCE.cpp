...
#pragma region CREATE SERIALIZED FILE IF DOES NOT EXIST
    ...
    //{
        ...
        int up, front;
        //FbxAxisSystem::ECoordSystem b = axis.GetUpVector();
        Axises.AxisSystem = axis.GetCoorSystem();
        Axises.UpAxis = axis.GetUpVector(up);
        Axises.FrontAxis = axis.GetFrontVector(front);
        if (Axises.AxisSystem == FbxAxisSystem::ECoordSystem::eRightHanded && Axises.UpAxis == FbxAxisSystem::EUpVector::eYAxis)
            Axises.AxisCoords = SystemTransformation::RHS_Y_UP();
        if (Axises.AxisSystem == FbxAxisSystem::ECoordSystem::eLeftHanded && Axises.UpAxis == FbxAxisSystem::EUpVector::eYAxis)
            Axises.AxisCoords = SystemTransformation::LHS_Y_UP();
        if (Axises.AxisSystem == FbxAxisSystem::ECoordSystem::eRightHanded && Axises.UpAxis == FbxAxisSystem::EUpVector::eZAxis)
            Axises.AxisCoords = SystemTransformation::RHS_Z_UP();
        if (Axises.AxisSystem == FbxAxisSystem::ECoordSystem::eLeftHanded && Axises.UpAxis == FbxAxisSystem::EUpVector::eZAxis)
            Axises.AxisCoords = SystemTransformation::LHS_Z_UP();
...
void MODEL_RESOURCES::RetrieveMeshes(FbxScene* scene)
{
    for (auto& n : Scenes.Nodes)
    {
        ...
        for (int ind = 0; ind < p_Count; ++ind)
        {
            ...
            for (int p_Pos = 0; p_Pos < 3; ++p_Pos)
            {
                ...
                //VERTEX& v = m.Vertices.at(v_Index);
                ...
                //m.Vertices.at(v_Index) = std::move(v);
                vt = v;
                bool exists{};
                int index{};
                for (auto& vt : m.Vertices)
                {
                    ...
                    if (v.position == vt.position)
                        p_same = true;
                    if (v.normal == vt.normal)
                        n_same = true;
                    if (v.tangent == vt.tangent)
                        t_same = true;
                    if (v.UV == vt.UV)
                        u_same = true;

                    //if (v.Bone_Properties.Indices[0] == vt.Bone_Properties.Indices[0] &&
                    //    v.Bone_Properties.Indices[1] == vt.Bone_Properties.Indices[1] &&
                    //    v.Bone_Properties.Indices[2] == vt.Bone_Properties.Indices[2] &&
                    //    v.Bone_Properties.Indices[3] == vt.Bone_Properties.Indices[3])
                    //    i_same = true;

                    //if (v.Bone_Properties.Weights[0] == vt.Bone_Properties.Weights[0] &&
                    //    v.Bone_Properties.Weights[1] == vt.Bone_Properties.Weights[1] &&
                    //    v.Bone_Properties.Weights[2] == vt.Bone_Properties.Weights[2] &&
                    //    v.Bone_Properties.Weights[3] == vt.Bone_Properties.Weights[3])
                    //    w_same = true;


                    if (p_same && n_same && t_same && u_same /*&& i_same && w_same*/)
                    {
                        exists = true;
                        break;
                    }
        ...
        //    {
        //        if (v.position == temp[a].position && v.normal == temp[a].normal && v.tangent == temp[a].tangent)
        //        {
        //            m.Indices[a] = ind;
                    //m.Subsets.at(submeshIndex[a]).indices.push_back(ind);
        //            break;
        //        } else {
        //            m.Subsets.at(submeshIndex[a]).indices.push_back(m.Indices[a]);
        //        }
        //        ++ind;
        //    }
        //}
    ...
    //    for (auto& v : m.Vertices)
    //    {
    //        VECTOR4 vector{ v.Bone_Properties.Weights[0] , v.Bone_Properties.Weights[1] , v.Bone_Properties.Weights[2] , v.Bone_Properties.Weights[3] };
    //        float total{ vector.x + vector.y + vector.z + vector.w };
    //        if (total <= 1.0f)
    //            continue;
    //        v.Bone_Properties.Weights[0] /= total;
    //        v.Bone_Properties.Weights[1] /= total;
    //        v.Bone_Properties.Weights[2] /= total;
    //        v.Bone_Properties.Weights[3] /= total;
...
void MODEL_RESOURCES::RetrieveMaterials(FbxScene* scene)
{
    // Node Count
    ...
    for (int ind = 0; ind < nCount; ++ind)
    {
        ...
        if (m_Count)
        {
            ...
            for (int /*Material index*/m_Index = 0; m_Index < m_Count; ++m_Index)
            {
...
void MODEL_RESOURCES::RetrieveInfleunces(FbxMesh* m, std::vector<std::vector<BONE_INFLUENCE>>& bi)
{
...
void MODEL_RESOURCES::RetrieveSkeleton(FbxMesh* m, SKELETON& s)
{
...
void MODEL_RESOURCES::RetrieveAnimations(FbxScene* s, float samplingRate)
{
    ...
    if (!a_StackCount)
    {
        ...
        //ANIMATION& a{ Animations.emplace_back() };
        //a.Name = "NO_ANIM";
        //a.SamplingRate = 60;
        //ANIMATION::KEYFRAME& kf{ a.Keyframes.emplace_back() };
        ////ANIMATION::KEYFRAME::NODE n{ kf.Nodes.emplace_back() };
        //kf.Nodes.resize(Scenes.Nodes.size());
        //XMStoreFloat4x4(&n.g_Transform, XMMatrixIdentity());
    }

    for (int ind = 0; ind < a_StackCount; ++ind)
        delete a_StackNames[ind];
...
bool MODEL_RESOURCES::AddAnimation(std::string anim_path, std::string animationName , float SamplingRate)
{
    FbxManager* manager{ FbxManager::Create() };
...
HRESULT MODEL_RESOURCES::AddMaterial(std::string mat_path, std::string model_name, MATERIAL_TYPE type, MATERIAL& m)
{
    std::string format;
...
void MODEL_RESOURCES::ResetDefaultTransform(MESH* m)
{
    m->BaseTransform = Convert::Identity();
}

/*-------------------------------------- - MODEL_RESOURCES SetShader()----------------------------------------------*/

//HRESULT MODEL_RESOURCES::SetShader(std::wstring model_name)
//{
//    Shader = SHADERMANAGER::Instance()->Load(model_name);
//    return Shader ? S_OK : E_INVALIDARG;
//}
